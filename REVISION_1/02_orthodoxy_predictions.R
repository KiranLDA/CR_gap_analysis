...
colnames(exceptional_wcvp_matched)[which(colnames(exceptional_wcvp_matched) %in%
                                           c("Exceptional.status",
                                             ...
                                             "PlantSearch.uncategorized.germplasm.collections"))] <- c("Exceptional_status",
...
length(unique(iucn_CR_predictions_wcvp_matched$taxon_name)) # 198
...
iucn_orthodox = read.csv(paste0(basepath,"revision_1/Model-results-2025-09-24_1.csv"))
for (i in 2:8){
  iucn_orthodox = rbind(iucn_orthodox,
                        read.csv(paste0(basepath,"revision_1/Model-results-2025-09-24_",i,".csv")))
}
...
iucn_storage_behaviour = iucn_wcvp_matched_orthodox %>% left_join(exceptional_wcvp_matched[,c("taxon_name", "taxon_authors", "wcvp_accepted_id","Exceptional_status",
                                                                                              ...
                                                                                              "PlantSearch_tissue_culture_collections", "PlantSearch_cryopreservation_collections",
                                                                                              "PlantSearch_uncategorized_germplasm_collections")],
                                                                  by=c("taxon_name","taxon_authors","wcvp_accepted_id"), relationship = "many-to-one")
...
# save the output
write.csv(iucn_storage_behaviour, paste0(basepath, "revision_1/iucn_orthodoxy_recalcitrance.csv"), row.names=FALSE)
...
length(unique(iucn_storage_behaviour$scientificName))
...
dim(CR_pred)
# write.csv(CR_pred, paste0(basepath, "revision_1/orthodox_to_match_predictions.csv"), row.names=FALSE )
...
# save the output
write.csv(CR_pred_to_add, paste0(basepath, "revision_1/CR_pred_orthodoxy_recalcitrance.csv"), row.names=FALSE)
...
write.csv(spp_banked_recalcitrant, paste0(basepath, "revision_1/spp_banked_recalcitrant.csv"), row.names=FALSE )
